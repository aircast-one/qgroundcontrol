...
UNDRAWN = [
    ...
    ("view.landingPattern", "a fixed-wing or VTOL landing pattern: this SITL is a quadrotor and "
                            ...
                            "here, so the LIVE rig cannot produce one. It does NOT follow that the "
                            "state is unreachable, which is what this reason said until it was "
                            "audited: the contract test sets the offline class under a qScopeGuard "
                            "and records fourteen leaf paths -- finalApproach, slopeStart, "
                            "loiterRadiusMetres, landingHeadingDegrees and the rest. So this is "
                            "buildable and testable WITHOUT the rig, and the old wording closed a "
                            "door that is open. Retake it with: git show HEAD:"
                            "test/Bridge/fixtures/view-shapes.json | python3 -c \"import sys,json; "
                            "print([p for p in json.load(sys.stdin)['_observed'] "
                            "if p.startswith('view.landingPattern')])\". PARITY GAP, recorded not "
                            "built: QML carries an EDITOR and "
