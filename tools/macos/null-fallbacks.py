...
ACCEPTED = {
    ("LinkFormCheck", "errorField"): "MEASURED LIVE over 13 tuples, and the `?? \"\"` is a "
        ...
        "named a field and every valid:true one sent null. So nil does not mean \"nothing is "
        "wrong\" -- it means \"no field this enum spells\", which today is only the valid case and "
        "tomorrow is also a refusal naming a field the head has no text box for. The core is "
        "adding errorField \"type\" for an unrecognised link type, and that arrives here as nil "
        "too: the right answer, because the picker fixes a bad type and host/port cannot, so "
        "refusing both obstructs without dead-ending the route to repair. The arm is kept "
