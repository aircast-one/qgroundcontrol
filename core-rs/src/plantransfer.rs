...
impl Transfer {
    pub fn new(apm: bool) -> Transfer {
        Transfer { apm, send_home: !apm, ..Transfer::default() }
    }
    ...
    pub fn on_item(&mut self, item: Item) -> Vec<Out> {
        ...
        if !self.take_expected(Expect::Item) {
            return match (self.apm, item.seq) {
                (true, 0) => vec![Out::HomePosition(item.params[4], item.params[5], item.params[6])],
                _ => Vec::new(),
            };
        }
        ...
        let params = match (item.command, self.send_home) {
            (CMD_DO_JUMP, false) => [item.params[0].trunc() + 1.0, item.params[1], item.params[2], item.params[3], item.params[4], item.params[5], item.params[6]],
            _ => item.params,
        };
        let seq = item.seq;
...
mod tests {
    ...
    fn a_write_skips_the_home_item_on_ardupilot_and_serves_requests_until_the_ack() {
        ...
        let mut refused = Transfer::new(false);
        ...
        let mut outside = Transfer::new(false);
    ...
    fn item_retries_use_the_short_timer_and_an_early_ack_fails_a_write() {
        ...
        let mut reading = Transfer::new(true);
        reading.load();
        reading.on_count(1);
        ...
        let mut early = Transfer::new(false);
        ...
        let mut stalled = Transfer::new(false);
    ...
    fn an_unrequested_first_item_on_ardupilot_is_the_home_position() {
        ...
        let mut px4 = Transfer::new(false);
