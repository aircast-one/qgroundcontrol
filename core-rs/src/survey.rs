...
pub fn survey_stats_view(backend: &dyn Backend, args: &[String]) -> Value {
    ...
    json!({
        ...
        "areaText": if area_m2 > 0.0 { format!("{:.0} {}", area.show(area_m2), area.name) } else { ABSENT.to_string() },
        "distanceMetres": distance_m,
        "distanceText": if distance_m > 0.0 { format!("{:.0} {}", distance.show(distance_m), distance.name) } else { ABSENT.to_string() },
        "footprintSide": side,
