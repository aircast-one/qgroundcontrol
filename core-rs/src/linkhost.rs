...
impl Transports {
    ...
    pub fn open(&mut self, config: LinkConfig) -> Result<LinkId, String> {
        let kind_name = match &config.kind {
            Kind::Udp { .. } => "udp",
            Kind::Tcp { .. } => "tcp",
            other => return Err(format!("the core does not own {other:?} links yet")),
        };
    ...
    pub fn open_json(&mut self, json: &str) -> Result<LinkId, String> {
        let value: Value = serde_json::from_str(json).map_err(|e| format!("not JSON: {e}"))?;
        self.open(linkconfig::from_json(&value)?)
    }
...
pub fn transports_view(_backend: &dyn crate::router::Backend, _args: &[String]) -> Value {
    TRANSPORTS.lock().unwrap().snapshot()
}
