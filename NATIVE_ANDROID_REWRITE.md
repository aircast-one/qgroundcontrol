...
telemetry strip read `0.0 ft` and `1421.7 ft` in the same run, which is the units
fix confirmed live a second time.
...
**A mistake worth recording because the tell was so quiet.** This entry was first
appended and committed to `aircast-android` — a `cd` earlier in the same shell
command redirected it, and the private-index commit created a second
`NATIVE_ANDROID_REWRITE.md` there. The only sign was `create mode 100644` in the
