...
| `unread-fields.py` | `python3 tools/unread-fields.py`. No device needed. Finds fields this head DECODES from a view and then never dereferences - the mirror of `served-unread.py`, which finds fields the core serves and the head never names. Ten are outstanding; each is either a field to delete or a feature to finish, and `LogsView.eraseWarning` is accepted with a reason. It self-checks against a known answer first and refuses to run if that answer is wrong, because the naive version of this check reported every property in the head as dead. |
