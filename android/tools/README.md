...
| — | **`probe.sh get` passes the path raw into the URL and the bridge does not url-decode it.** A parenthesised view argument works (`view.geoTag(/path/x.tlog)`); the same request through `curl --data-urlencode` returns `kind:null`, because `%28` never becomes `(`. A path containing spaces therefore cannot be asked about through the debug api at all — which includes every telemetry log this app writes, since its directory is "Aircast QGC Daily". The head is unaffected: it calls the bridge over JNI, where no url encoding exists. |
