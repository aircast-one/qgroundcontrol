...
| — | **Never `pkill` a script name here, and a path anchor does not save you.** Four sessions share this checkout, so `apmvehicle.py`, `detfeed.py` and `udptcp.py` name every session's copy, and `pkill -f gst-launch-1.0` matched every gstreamer on the machine. Anchoring on `$S/` fails BOTH ways: `regress.sh` launches with an absolute `$S` so the anchor still matches a peer, and it MISSES the same script started relatively, which is how it is usually run by hand. A killed fake stops sending, which reads as an unbuilt feature rather than as interference - expensive to chase. So: keep the PID you started (`SIM=$!`) and kill that, and where a contended port exists guard it with `lsof -nP -iTCP:<port> -sTCP:LISTEN`, which sees the holder however it was launched. `regress.sh` and `detrig.sh` both refuse rather than clearing someone else's process (`e5e2cd812`). |
