...
| — | **The emulator is a real fallback when the handset is unplugged.** `ANDROID_SERIAL=emulator-5554` works with `installDebug`, `probe.sh`, `ui.sh` and `adb reverse`, so anything not needing a vehicle or USB can still be measured: empty states, settings pages, and the whole ADSB traffic path, because the SBS-1 feed is TCP. It is also where the packet-radio startup crash was found. Its profile is imperial, which makes it the cheapest way to see a unit conversion that a metric handset hides. |
