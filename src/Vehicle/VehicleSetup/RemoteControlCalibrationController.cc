...
void RemoteControlCalibrationController::_setupCurrentState()
{
    ...
    _setNextEnabled(state.nextButtonFn != nullptr);
...
QVariantList RemoteControlCalibrationController::rcValues() const
{
    QVariantList values;
    values.reserve(_chanCount);
    for (int channel = 0; channel < _chanCount; channel++) {
        values.append(_channelRawValue[channel]);
    }
    return values;
}

void RemoteControlCalibrationController::_setStatusText(const QString &text)
{
    if (_statusText != text) {
        _statusText = text;
...
void RemoteControlCalibrationController::_setNextText(const QString &text)
{
    if (_nextText != text) {
        _nextText = text;
...
void RemoteControlCalibrationController::_setNextEnabled(bool enabled)
{
...
void RemoteControlCalibrationController::_setCancelEnabled(bool enabled)
{
