...
GstElement* buildWhepSource(const QUrl& sourceUrl, const Config& config, guint latencyMs)
{
    ...
    g_object_set(source, "whep-endpoint", endpointUrl.toString().toUtf8().constData(), "video-caps", videoCaps,
...
GstElement* create(const QString& uri, const Config& config)
{
    ...
    const bool isWhep = isWhepUri(uri);
    ...
    do {
        ...
        if (isUdpH265) {
            ...
            if (!linkSourceToParser(bin, upstream, binParser, config, latencyMs, isTcpMPEGTS || isUdpMPEGTS, isRtsp || isWhep)) {
                break;
            }
        }
