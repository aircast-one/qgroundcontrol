...
void PlanMasterController::_shiftSnapshot(QList<QByteArray>& from, QList<QByteArray>& to, const QString& failureMessage)
{
    ...
    if (!_restoreSnapshot(from.last(), errorString)) {
        from.removeLast();
        QGC::showAppMessage(failureMessage.arg(errorString));
        QString ignored;
...
bool PlanMasterController::_restoreSnapshot(const QByteArray& snapshot, QString& errorString)
{
    ...
    _restoring = true;
    const bool loaded = _loadPlanJson(snapshot, errorString);
    _restoring = false;
    if (!loaded) {
        return false;
    }
...
void PlanMasterController::_notifyDirty(void)
{
    const bool overallDirty = dirty();
    if (_previousOverallDirty == overallDirty) {
        return;
    }
    _previousOverallDirty = overallDirty;
    if (!overallDirty && !_restoring) {
        _cleanSnapshot = _planSnapshot();
    }
    emit dirtyChanged(overallDirty);
}
